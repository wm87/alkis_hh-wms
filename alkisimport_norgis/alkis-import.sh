...
import() {
	...
	*)
	...
	echo "RUNNING: ogr2ogr -skipfailures -f $DRIVER $opt $sf_opt -update -append \"$DST\" $CRS \"$dst1\"" | sed -Ee 's/password=\S+/password=*removed*/'
	eval ogr2ogr -skipfailures -f $DRIVER $opt $sf_opt -update -append \"$DST\" $CRS \"$dst1\"
	local r=$?
	t1=$(bdate +%s)
...
GDAL_VERSION=$(
	unset CPL_DEBUG
	/opt/gdal/bin/ogr2ogr --version
)
echo $GDAL_VERSION

IFS=". " read gdal GDAL_MAJOR GDAL_MINOR _ <<<$GDAL_VERSION
if [ "$gdal" != "GDAL" ] || ((GDAL_MAJOR < 2 || (GDAL_MAJOR == 2 && GDAL_MINOR < 3))); then
